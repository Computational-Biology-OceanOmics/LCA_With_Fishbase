...
class TestCalculateLCAFunctional(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(self.test_input.exists(), f"Test input not found: {self.test_input}")
        self.assertTrue(self.expected_output.exists(), f"Expected output not found: {self.expected_output}")
    ...
    def test_end_to_end_lca_calculation(self):
        ...
        cmd = [
            ...
            "-f", str(self.test_input),
            "-o", str(self.test_output),
            "--missing_out", str(self.test_missing),
        ...
        self.assertEqual(expected_asvs, actual_asvs, 
        ...
        # compare each row
        for i, (expected_row, actual_row) in enumerate(zip(expected_df.itertuples(), actual_df.itertuples())):
            with self.subTest(row=i, asv=expected_row.ASV_name):
                ...
                self.assertAlmostEqual(float(actual_row.PercentageID), 
                                     float(expected_row.PercentageID), places=2)
                self.assertAlmostEqual(float(actual_row.Coverage), 
                                     float(expected_row.Coverage), places=2)
                
                # species_In_LCA might have different ordering but same content
                expected_species = set(expected_row.Species_In_LCA.split(', ')) if expected_row.Species_In_LCA else set()
                actual_species = set(actual_row.Species_In_LCA.split(', ')) if actual_row.Species_In_LCA else set()
                self.assertEqual(actual_species, expected_species)
                
                # sources might have different ordering but same content
                expected_sources = set(expected_row.Sources.split(', ')) if expected_row.Sources else set()
                actual_sources = set(actual_row.Sources.split(', ')) if actual_row.Sources else set()
    ...
    def test_invalid_input_file(self):
        ...
        self.assertNotEqual(result.returncode, 0, 
                           "Script should fail with non-existent input file")
    ...
    def test_invalid_parameters(self):
        ...
        self.assertNotEqual(result.returncode, 0, 
                           "Script should fail with invalid pident > 100")
        
        cmd = [
            sys.executable, str(self.script_path),
            "-f", str(self.test_input),
            "-o", str(self.test_output),
            "--cutoff", "-1.0",  # Invalid: negative
            "--log_level", "WARNING"
        ]
        ...
        self.assertNotEqual(result.returncode, 0, 
                           "Script should fail with negative cutoff")
